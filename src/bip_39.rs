...
pub fn append_checksum(entropy_bytes: &mut Vec<u8>) {
    let checksum = sha256(entropy_bytes);
    let checksum_to_bits: String = checksum
        .iter()
        .map(|byte| format!("{:08b}", byte))
        .collect();
    let first_four_bits = &checksum_to_bits[..4];
    let first_four_bits_value = u8::from_str_radix(first_four_bits, 2).unwrap();
    let checksum_nibble = first_four_bits_value & 0b1111;
    entropy_bytes.push(checksum_nibble << 4);
}

pub fn extract_11bit_chunks(data: &[u8]) -> Vec<u16> {
    ...
    while bit_pos + 11 <= total_bits {
        let mut chunk: u16 = 0;
        for i in 0..11 {
            let current_bit_pos = bit_pos + i;
            let current_byte_index = current_bit_pos / 8;
            let current_bit_offset = current_bit_pos % 8;

            let bit = (data[current_byte_index] >> (7 - current_bit_offset)) & 1;
